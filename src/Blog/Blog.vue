<template>
  <div class="Blog">
    ...
    <!--博客内容列表-->
    <el-card v-for="(item,index) in $store.state.bloglist" class="box-card" id="BlogItem" :key="index" >
      <div @click="showBlogDetail(index)">
        <!--标题-->
        <p class="Blogtitle"><strong>{{item.blogtitle}}</strong></p>
        <!--博客简要介绍-->
        ...
        <div class="iconset">
          ...
          <div class="label">
           ...
           <!--你要放入的标签-->
            <div class="insertlabel" v-for="(itemone, indexone) in item.tagList" :key="indexone">
              <!-- <p :style="{color:itemone.color}">{{itemone.tagname}}</p> -->
         ...
         <div class="insertform" v-for="(itemtwo, indextwo) in item.selectedkind" :key="indextwo">
                <!-- <p :style="{color:itemtwo.color}">{{itemtwo.name}}</p> -->
    ...
    <el-card class="box-card" id="BlogPersonalInfo">
      ...
      <!--博客个人头像-->
      <el-tooltip class="item" effect="dark" content="博客管理" placement="right" style="cursor: pointer">
    ...
    <!--博客目录-->
    <Blogcatalog class="Blogcatalog" v-show="flagBlogcatalog === true"></Blogcatalog>
  </div>
</template>
